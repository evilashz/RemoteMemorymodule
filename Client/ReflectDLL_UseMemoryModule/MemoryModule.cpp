...
static void CopySections(const unsigned char* data, PIMAGE_NT_HEADERS old_headers, PMEMORYMODULE module)
{
	...
	for (i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++) {
		if (section->SizeOfRawData == 0) {
			...
			if (size > 0) {
				dest = (unsigned char*)PigVirtualAlloc(codeBase + section->VirtualAddress,
					size,
					MEM_COMMIT,
					PAGE_READWRITE);
...
#define imageOffset 0
	...
	for (i = 0; i < module->headers->FileHeader.NumberOfSections; i++, section++) {
		...
		if (size > 0) {
			...
			if (PigVirtualProtect((LPVOID)((POINTER_TYPE)section->Misc.PhysicalAddress | imageOffset), size, protect, &oldProtect) == 0)
#ifdef DEBUG_OUTPUT
...
HMEMORYMODULE MemoryLoadLibrary(const void* data)
{
	...
	code = (unsigned char*)PigVirtualAlloc((LPVOID)(old_header->OptionalHeader.ImageBase),
	...
	if (code == NULL) {
		...
		code = (unsigned char*)PigVirtualAlloc(NULL,
			old_header->OptionalHeader.SizeOfImage,
			MEM_RESERVE,
			PAGE_READWRITE);
		if (code == NULL) {
